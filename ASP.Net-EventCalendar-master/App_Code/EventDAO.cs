...
public class EventDAO
{
    ...
    public static List<CalendarEvent> getEvents(DateTime start, DateTime end)
    {
        ...
        using (DBContextDataContext db = new DBContextDataContext())
        {
            ...
            var qry = (from x in db.Appointments where x.AppointmentDate >= start && x.AppointmentComplete <= end select x);

            foreach (var time in qry)
            {
                CalendarEvent cevent = new CalendarEvent();
                cevent.id = Convert.ToInt32(time.AppointmentID);
                cevent.title = time.AppointmentTitle;
                cevent.description = time.AppointmentDescription;
                cevent.start = Convert.ToDateTime(time.AppointmentDate);
                cevent.end = Convert.ToDateTime(time.AppointmentComplete);
                events.Add(cevent);
            }
    ...
    public static int addEvent(CalendarEvent cevent)
    {
        ...
        using (DBContextDataContext db = new DBContextDataContext())
        {
            ...
            var first = db.Appointments.OrderByDescending(x => x.AppointmentID).First();

            int key = Convert.ToInt32(first);
